...
const executeWithRetry = async (requestFn, maxAttempts = API_RETRY_ATTEMPTS) => {
  let lastError;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.log(`Intento ${attempt} fallido, reintentando en ${API_RETRY_DELAY}ms...`);
...
const apiExternaService = {
  ...
  async obtenerReferencias(token) {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const posibleArray = Object.values(data).find(value => Array.isArray(value));
        if (posibleArray) {
          return posibleArray;
        }
      }
      ...
      console.log('Estructura de datos inesperada de la API, usando datos mock');
  ...
  async obtenerDetalleReferencia(referenciaId, token) {
    try {
      ...
      const requestBody = {
        "Pedimento": referenciaId.includes('-') ? referenciaId.split('-')[1] : referenciaId,
        "TipoFecha": 1,
        "FechaInicial": "01/01/2022",
        "FechaFinal": "31/12/2025"
      };
      ...
      if (referencia) {
        ...
        console.log(`No se encontró la referencia ${referenciaId} en la respuesta, usando datos mock`);
        const mockData = getMockReferencias();
        return mockData.find(ref => ref.id === referenciaId) || mockData[0];
      }
    } catch (error) {
      console.error(`Error al obtener detalle de referencia ${referenciaId}:`, error);
  ...
  async buscarReferencias(termino, token) {
    try {
      ...
      // Si no se pudo extraer un array, usar datos mock
      console.log('Estructura de datos inesperada de la API, usando datos mock para búsqueda');
      const mockData = getMockReferencias();
      return mockData.filter(
        ref => ref.id.toLowerCase().includes(termino.toLowerCase())
      );
    } catch (error) {
      console.error(`Error al buscar referencias con término "${termino}":`, error);
